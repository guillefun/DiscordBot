...
module.exports.run = async (client, message, args) =>{
    const member = message.mentions.members.first() 
                ...
                || message.member;
    const data = await client.db.get(`level-${message.guild.id}-${message.author.id}`);
    ...
    const url_background = await client.db.get(`banner-${message.guild.id}-${message.author.id}`);
    var background="";
    if(!url_background)
       background = await loadImage("https://png.pngtree.com/thumb_back/fw800/back_our/20190620/ourmid/pngtree-auto-show-board-background-material-image_162844.jpg");
    else
